...
public class CatalogoService {
    ...
    public JuegoResponseDTO agregarJuego(JuegoRequestDTO dto) {
        ...
        if (juegoRepository.existsByTituloIgnoreCase(dto.getTitulo())) {
            ...
            throw new BusinessRuleException("Ya existe un juego con el titulo: " + dto.getTitulo());
        }
    ...
    public JuegoResponseDTO buscarPorId(Long id) {
        ...
        Juego juego = juegoRepository.findById(id)
                .orElseThrow(() -> {
                    ...
                    return new ResourceNotFoundException("No encontrado: " + id);
                });
        return toDTO(juego);
    }
    ...
    public JuegoResponseDTO descontarStock(Long id, Integer cantidad) {
        ...
        if (cantidad <= 0) {
            throw new BusinessRuleException("La cantidad a descontar debe ser mayor a 0");
        }
        Juego juego = juegoRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Juego no encontrado con ID: " + id));
        if (juego.getStock() < cantidad) {
            throw new BusinessRuleException("Stock insuficiente para el juego: " + juego.getTitulo());
        }
    ...
    public JuegoResponseDTO agregarStock(Long id, Integer cantidad) {
        ...
        if (cantidad <= 0) {
            throw new BusinessRuleException("La cantidad a agregar debe ser mayor a 0");
        }
        Juego juego = juegoRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Juego no encontrado con ID: " + id));
